...
export default function User(props): User {
    ...
    >
      <Link href={`/${props.user.username}/${type.slug}`}>
        <a className="block px-6 py-4">
          ...
          ></div>
          <h2 className="inline-block font-medium dark:text-white">
            {type.title}
          </h2>
          <p className="inline-block text-gray-400 dark:text-gray-100 ml-2">
            {type.description}
          </p>
        </a>
      </Link>
  ...
  return (
    isReady && (
      <div>
        <Head>
          <title>{props.user.name || props.user.username} | Lexington</title>
