...
export default function Type(props): Type {
  ...
  useEffect(() => {
    handleToggle24hClock(
      localStorage.getItem("timeOption.is24hClock") === "true"
    );
    telemetry.withJitsu((jitsu) =>
      jitsu.track(telemetryEventTypes.pageView, collectPageParameters())
    );
  }, [telemetry]);

  const changeDate = (date: Dayjs) => {
    telemetry.withJitsu((jitsu) =>
      jitsu.track(telemetryEventTypes.dateSelected, collectPageParameters())
    );
      ...
      <div>
        <Head>
          <title>
            ...
            {props.user.name || props.user.username} | Lexington
          </title>
          <meta
            name="title"
            content={
          ...
          <meta
            ...
            content={props.eventType.description}
          />
          <meta
            property="og:image"
            content={
        ...
        >
          <div className="dark:bg-gray-800 bg-white sm:shadow sm:rounded-lg">
            <div className="sm:flex px-4 py-5 sm:p-4">
              <div
                className={
                  "pr-8 sm:border-r sm:dark:border-gray-900 " +
              ...
              >
                ...
                />
                <h2 className="font-medium dark:text-gray-300 text-gray-500">
                  {props.user.name}
                </h2>
                ...
                <p className="dark:text-gray-200 text-gray-600 mt-3 mb-8">
                  {props.eventType.description}
                </p>
              </div>
              <DatePicker
                ...
                periodCountCalendarDays={
                  props.eventType?.periodCountCalendarDays
                }
                weekStart={props.user.weekStart}
                onDatePicked={changeDate}
                workingHours={props.workingHours}
                organizerTimeZone={
                  props.eventType.timeZone || props.user.timeZone
                }
...
export const getServerSideProps: GetServerSideProps = async (
  ...
  const date =
    Array.isArray(dateQuery) && dateQuery.length > 0
      ? dateQuery.pop()
      : dateQuery;
  ...
  const getWorkingHours = (providesAvailability) =>
    ...
    providesAvailability.availability.length
      ? providesAvailability.availability
      : null;
  ...
  const workingHours: [] =
    ...
    [
      ...
      (availability): boolean => typeof availability["days"] !== "undefined"
    );
